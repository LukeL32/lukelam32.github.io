...
df <- read_csv('titanic.csv')
subject <- mode_missing(df$Embarked)
max(subject)
```
...
viz <- function(df)
{
  ...
  {
    df %>% ggplot(aes(x = df[[1]], y = df[[2]]))+
      geom_boxplot()+
      labs(x = names(df)[1], y = names(df)[2])
  }
  else if ((!is.numeric(df[[1]])&is.numeric(df[[2]])))
  {
    df %>% ggplot(aes(x = df[[2]], fill = df[[1]]))+
      geom_boxplot()+
      labs(x = names(df)[2], fill = names(df)[1])
  }
  else 
  {
...
viz <- function(df)
{
  ...
  {
    df %>% ggplot(aes(x = df[[1]], fill = df[[2]]))+
