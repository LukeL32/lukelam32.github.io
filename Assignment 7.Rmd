...
```{r}
df %>% filter(Embarked == "S") %>% ggplot()+ 
  geom_bar(mapping=aes(x=Pclass))+
  facet_wrap(~Embarked+Sex)
```

3. Make a plot using `position ='dodge'`.
```{r}
df %>% ggplot()+ 
  geom_bar(mapping=aes(x=Sex), position = 'dodge')
```

4. Make a plot using `position ='fill'`. 
```{r}
df %>% ggplot()+ 
  geom_bar(mapping=aes(x=Sex,fill=Embarked), position = 'fill')
```
...
df %>% group_by(Pclass) %>% 
  summarise(average_age=mean(Age, na.rm=TRUE)) %>% 
  ggplot()+ 
  geom_col(mapping=aes(x=Pclass, y=average_age)) + theme_dark()
```
...
df %>% filter(Pclass == 1) %>%
  ggplot()+
  geom_boxplot(mapping=aes(x=Age,group=Sex))
```
...
```{r}
df %>% 
  filter(location %in% c('Japan', 'Nepal', 'Cyprus', 'Hong Kong', 'South Korea')) %>% 
  group_by(date, location) %>% 
  summarise(total_vac=sum(total_vaccinations, na.rm=TRUE)) %>% 
  ggplot()+
  geom_point(mapping=aes(x=date, 
                         y=total_vac, 
                         color=location))
...
```{r}
df %>% 
  filter(location %in% c('Argentina', 'Peru', 'Ecuador', 'Uruguay', 'South Korea'), date<'2022-01-01', date>='2021-01-01') %>% 
  group_by(date, location) %>% 
  summarise(total_vac=sum(total_vaccinations, na.rm=TRUE)) %>% 
  ggplot()+
  geom_point(mapping=aes(x=date, 
